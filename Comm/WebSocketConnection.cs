...
namespace MobileDeliveryServer.Comm
{
    public class WebSocketConnection : IWebSocketConnection
    {
        ...
        public void CreateHandler(IEnumerable<byte> data)
        {
            ...
            ConnectionInfo = WebSocketConnectionInfo.Create(request, Socket.RemoteIpAddress, Socket.RemotePort, subProtocol, PM);
            ...
            return Socket.Send(bytes, () =>
            {
                ...
                if (callback != null)
                    callback();
                              ...
                              {
                                  if (e is IOException)
                                      Logger.Debug("Failed to send. Disconnecting.", e);
                                  else
                                      Logger.Info("Failed to send. Disconnecting.", e);
                                  CloseSocket();
                              });
        }
